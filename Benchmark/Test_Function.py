...
class Hartmann_complicated(SyntheticTestFunction):
    ...
    def evaluate_stochastic(self, X, sample_size=65536):
        ...
        X = X.reshape(1, -1)
        engine = SobolEngine(1)
        context = torch.zeros((sample_size, 1))
        sobol_samples = engine.draw(sample_size)
        
        for i in range(sample_size):
            k = torch.randint(low=0, high=8, size=(1,)).item()
            if k == 7:
                context[i] = (torch.tensor([0.2]) + torch.tensor(0.02)*torch.tan(torch.pi * (sobol_samples[i] - 0.5))).item()
            elif k == 6:
                context[i] = (torch.tensor([0.8]) + torch.tensor(0.02)*torch.tan(torch.pi * (sobol_samples[i] - 0.5))).item()
            else:
                std_sample = torch.erfinv(2 * sobol_samples[i] - 1) * math.sqrt(2)
                context[i] = (self.center[k] + self.std[k] * std_sample).item()
